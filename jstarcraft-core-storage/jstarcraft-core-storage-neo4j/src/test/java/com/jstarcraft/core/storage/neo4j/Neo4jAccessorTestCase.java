...
public class Neo4jAccessorTestCase {
	...
	public void testCRUD() {
		...
		// 查询指定范围的主键与对象
		Map<Integer, Object> id2Moneys = accessor.queryIdentities(MockNode.class, "money", new StorageCondition<>(ConditionType.All));
		Assert.assertThat(id2Moneys.size(), CoreMatchers.equalTo(size));
		List<MockNode> objects = accessor.queryInstances(MockNode.class, "money", new StorageCondition<>(ConditionType.All));
		for (MockNode object : objects) {
			Assert.assertThat(object.getMoney(), CoreMatchers.equalTo(id2Moneys.get(object.getId())));
		}

		id2Moneys = accessor.queryIdentities(MockNode.class, "money", new StorageCondition<>(ConditionType.Equal, 0));
		Assert.assertThat(id2Moneys.size(), CoreMatchers.equalTo(1));
		objects = accessor.queryInstances(MockNode.class, "money", new StorageCondition<>(ConditionType.Equal, 0));
		for (MockNode object : objects) {
			Assert.assertThat(object.getMoney(), CoreMatchers.equalTo(id2Moneys.get(object.getId())));
		}

		id2Moneys = accessor.queryIdentities(MockNode.class, "money", new StorageCondition<>(ConditionType.Between, 1, 50));
		Assert.assertThat(id2Moneys.size(), CoreMatchers.equalTo(50));
		objects = accessor.queryInstances(MockNode.class, "money", new StorageCondition<>(ConditionType.Between, 1, 50));
		for (MockNode object : objects) {
			Assert.assertThat(object.getMoney(), CoreMatchers.equalTo(id2Moneys.get(object.getId())));
		}

		id2Moneys = accessor.queryIdentities(MockNode.class, "money", new StorageCondition<>(ConditionType.In, 25, 50, 75));
		Assert.assertThat(id2Moneys.size(), CoreMatchers.equalTo(3));
		objects = accessor.queryInstances(MockNode.class, "money", new StorageCondition<>(ConditionType.In, 25, 50, 75));
		for (MockNode object : objects) {
			Assert.assertThat(object.getMoney(), CoreMatchers.equalTo(id2Moneys.get(object.getId())));
		}
